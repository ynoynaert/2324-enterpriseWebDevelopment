	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.csrfTokenRepository(new HttpSessionCsrfTokenRepository()))
				.authorizeHttpRequests(requests -> requests.requestMatchers("/login**").permitAll()
						...
						.requestMatchers("/*").hasAnyRole("USER", "ADMIN").requestMatchers("/sports/*")
						.hasAnyRole("USER", "ADMIN").requestMatchers("/sports/*/addCompetition").hasRole("ADMIN")
						.requestMatchers("/sports/*/buyTickets/*").hasRole("USER").requestMatchers("/sports/*/tickets")
						.hasRole("USER"))
